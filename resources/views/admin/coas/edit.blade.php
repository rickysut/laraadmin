...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.coa.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.coas.update", [$coa->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="coa_code">{{ trans('cruds.coa.fields.coa_code') }}</label>
                <input class="form-control {{ $errors->has('coa_code') ? 'is-invalid' : '' }}" type="number" name="coa_code" id="coa_code" value="{{ old('coa_code', $coa->coa_code) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coa_name') ? 'is-invalid' : '' }}" type="text" name="coa_name" id="coa_name" value="{{ old('coa_name', $coa->coa_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('coa_parent') ? 'is-invalid' : '' }}" name="coa_parent_id" id="coa_parent_id">
                    @foreach($coa_parents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('coa_parent_id') ? old('coa_parent_id') : $coa->coa_parent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

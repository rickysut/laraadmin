    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.backdate-banpems.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kd_akun_id') ? 'is-invalid' : '' }}" name="kd_akun_id" id="kd_akun_id" required>
                    @foreach($kd_akuns as $id => $entry)
                        <option value="{{ $id }}" {{ old('kd_akun_id') == $id ? 'selected' : '' }}>{{ $id }} - {{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom_pagu') ? 'is-invalid' : '' }}" type="number" name="nom_pagu" id="nom_pagu" value="{{ old('nom_pagu', '0') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nom_realisasi') ? 'is-invalid' : '' }}" type="number" name="nom_realisasi" id="nom_realisasi" value="{{ old('nom_realisasi', '0') }}" step="0.01" required>
